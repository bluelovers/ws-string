...
import { _table } from '../../src/data/_table';
import { tableDefaultInclude } from '../../src/data/defaults';
import { EnumFullHalfTableType } from '../../src/index';
import { ITableObject } from '../../src/types';
import { _toFullWidth } from '../../src/core';
...

{
	...
	function fn(v)
	{
		...
		if (r[0].from && r[0].to)
		{
			r[1].from = _toFullWidth(r[0].from);
			r[1].to = _toFullWidth(r[0].to);
		}
...
outputFileSync(join(__ROOT, 'src', 'data', 'table.ts'), lines.join('\n\n'))
